...
class maps:
                ...
                def __init__(self, centerLat, centerLng, zoom,vehicle,div_b,div_s):
                ...
                def drawpaths(self, f, paths):
                                                #print path
                                        for i in paths:
                                            self.drawPolyline(f,i[0][:-1], i[1],strokeColor = i[0][-1])
                ...
                def drawPolyline(self,f,path,\
                                ...
                                f.write('];\n')
                                f.write('var n = PolylineCoordinates.length;\n')
                                f.write('var dat=new Array();\n')
                                f.write('var an1=new Array();\n')
                                for i in range(len(dates)):
                                        f.write("dat[%d]='%s';\n" % (i,dates[i][0]))
                                        f.write("an1[%d]='%s';\n" % (i,dates[i][1]))
                                f.write('for(var i=0;i<n-1;++i) {\n')
                                f.write('var latlon=new Array();\n')
                                f.write('for(var j=i;j<i+2 && j<n;++j) { \n')
                                f.write('latlon[j-i]=PolylineCoordinates[j];\n')
                                f.write('} \n')
                                f.write("var date=dat[i+1];\n")
                                f.write('\n')
                                f.write('var symbol = { \n')
                                f.write('path: google.maps.SymbolPath.FORWARD_OPEN_ARROW\n')
                                f.write('};\n')
                                f.write("var vcl='%s';\n" % (self.vehicle))
                                f.write("var contentString = vcl+'<br>'+'an1('+an1[i+1]+') '+date+'<br>/'+String(latlon[1]);\n")
                                f.write('var marker = new google.maps.Marker({\n')
                                f.write('position:latlon[1],\n')
                                f.write('icon: {\n')
                                f.write('path: google.maps.SymbolPath.CIRCLE,\n')
                                f.write('scale:2')
                                f.write('},\n')
                                ...
                                f.write("col='black';\n")
                                f.write('}\n')
                                f.write('addwindow(map,marker,contentString)\n')
                                f.write('var Path = new google.maps.Polyline({\n')
                                f.write('clickable: %s,\n' % (str(clickable).lower()))
                                f.write('geodesic: %s,\n' % (str(geodesic).lower()))
                                f.write('path: latlon,\n')
                                f.write('strokeColor: col,\n')
                                ...
                                f.write(' } \n')
                                f.write('function addwindow(map,marker,contentString) {\n')
                                f.write('var rcontent=contentString;\n')
                                f.write('if(i==3) {\n')
                                if self.div_b:
                                    f.write("rcontent=rcontent+'<br>'+'Bearing Jump(Degree)'+'('+'%s'+'->'+'%s'+')';\n" % (self.div_b[0],self.div_b[1]))
                                if self.div_s:
                                    f.write("rcontent=rcontent+'<br>'+'Speed_Jump(Km/hr)'+'('+'%s'+'->'+'%s'+')';\n" % (self.div_s[0],self.div_s[1]))
                                f.write('}\n')
                                f.write('var infowindow= new google.maps.InfoWindow( {\n')
                                f.write('content:rcontent,\n')
